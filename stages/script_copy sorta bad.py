...
def item_animations():
    ...
    for x in item_x:
        ...
        if orbs_collected >= 80: bullet_piercing = True
...
def render_enemy(game_window, enemy_frame, enemy):
    global enemies_displayed
    if enemies_displayed < 10:
...
def create_enemies(path: str, wave, wave_num):
    ...
    with open(path, "r") as reader:
        ...
        for n, line in enumerate(reader):
            ...
            if not line or line == "EOF":  # this is causing the problem
            ...
            for i in range(len(values)):
                ...
                # x pos
                if n + wave == 0 + wave:
                    enemies_dict[i + wave]['x'] = values[i]
                
                # y pos
                elif n + wave == 1 + wave:
                    enemies_dict[i + wave]['y'] = values[i]
                
                # x speed
                elif n + wave == 2 + wave:
                    enemies_dict[i + wave]['x_speed'] = values[i]

                # y speed
                elif n + wave == 3 + wave:
                    enemies_dict[i + wave]['y_speed'] = values[i]

                # bullet x speed
                elif n + wave == 4 + wave:
                    enemies_dict[i + wave]['bullet_x_speed'] = values[i]

                # bullet y speed
                elif n + wave == 5 + wave:
                    enemies_dict[i + wave]['bullet_y_speed'] = values[i]

                # bullet fire rate
                elif n + wave == 6 + wave:
                    enemies_dict[i + wave]['bullet_delay'] = values[i]

                # enemy type
                elif n + wave == 7 + wave:
                    enemies_dict[i + wave]['enemy_type'] = values[i]
                    total_enemies += 1

                enemies_dict[i + wave]['dead'] = False
...
def game_loop(path: str):
    global player_bullets, player_x, player_y, play_game, home_page, play_game, live_num, in_battle, player_rect, enemies_killed, deaths, background, items

    enemies_dict = create_enemies(path, 0, 1)
    ...
    while in_battle:
        ...
        if time_passed + 10000 < py.time.get_ticks():
            enemies_dict = create_enemies(path, 8, 2)
        ...
        for key, enemy in enemies_dict.items():
            ...
            if not enemy['dead']:
                ...
                if enemy['enemy_rect'].colliderect(player_rect):
                    in_battle = False
                    live_num -= 1
                    home_page = True
                    if live_num == -1:
